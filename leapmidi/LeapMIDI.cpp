...
namespace LeapMIDI {
    ...
    void Listener::initGestures() {
        if (gestureRecognizers)
            delete gestureRecognizers;
        
        gestureRecognizers = new std::vector<Gesture *>();
        
        // instantiate recognizers
        GestureBall *ballGesture = new GestureBall();
    ...
    Listener::~Listener() {
        if (gestureRecognizers) {
            ...
            for (std::vector<Gesture *>::iterator it = gestureRecognizers->begin(); it != gestureRecognizers->end(); ++it) {
                delete *it;
            }
            delete gestureRecognizers;
        }
